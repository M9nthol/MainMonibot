...
def handle_start(message):
    ...
    button_tab = types.InlineKeyboardButton(text="Таблица с ценами для торговли", callback_data="tab")
    ...
    markup.add(button_tips, button_auction, button_hist, button_tab)
...
def handle_tab_callback(call):
    from auc import user_threads
    chat_id = call.message.chat.id
    try:
        # Проверяем, является ли пользователь участником частного чата
        member = bot.get_chat_member(chat_id=-1002098126171, user_id=call.from_user.id)

        # Если пользователь является участником частного чата
        if member.status != 'left':  #Проверяем, что пользователь не покинул чат
            # Формируем кнопку для перехода к таблице
            markup = types.InlineKeyboardMarkup()
            button_tab = types.InlineKeyboardButton(text="Перейти к таблице",
                                                    url="https://docs.google.com/spreadsheets/d/17Z05tiAX00VFRPvoK0Qy1oYrUhXbd4GbymcnT7gkFwM/edit?gid=0#gid=0")
            markup.add(button_tab)

            # Отправляем сообщение с кнопкой
            bot.send_message(chat_id, "Проверка прошла успешно!", reply_markup=markup)
        else:
            # Пользователь не в чате, отправляем сообщение с просьбой подписаться на канал
            bot.send_message(chat_id,
        ...
        # Обрабатываем возможные ошибки при проверке членства
        bot.answer_callback_query(callback_query_id=call.id, text="Произошла ошибка. Попробуйте снова.")
        print(f"Ошибка при проверке пользователя в чате: {e}")
